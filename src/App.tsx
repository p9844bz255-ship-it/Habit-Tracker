...
export default function App() {
  // Configuration Toggle to show/hide the Schedule Dashboard tab (can be flipped to true to re-enable)
  const isScheduleDashboardEnabled = false;

  const [isLoggedIn, setIsLoggedIn] = useState(true); // Default immediately to logged in view
  ...
  const buildPlansAndTeachers = (jadwalList: any[], savedPlans: any[]) => {
    ...
    teacherNames.forEach((name, index) => {
      ...
      // Determine divisi from schedules in the database
      let teacherDivisi = "-";
      const validSchedulesWithDivisi = teacherSchedules.filter((js) => js.divisi && js.divisi.trim() !== '-');
      if (validSchedulesWithDivisi.length > 0) {
        teacherDivisi = validSchedulesWithDivisi[0].divisi;
      } else if (teacherSchedules[0] && teacherSchedules[0].divisi) {
        teacherDivisi = teacherSchedules[0].divisi;
      }
      ...
      if (!jList || jList.length === 0) {
        ...
        for (const t of INITIAL_TEACHERS) {
          for (const l of t.lessons) {
            ...
            seedPromises.push(setDoc(doc(db, "jadwal_biasa", id), seedSchedule));
          }
        }
        ...
        for (const lp of INITIAL_LESSON_PLANS) {
          ...
          seedPromises.push(setDoc(doc(db, "lesson_plans", planId), {
            id: planId,
            class: lp.class,
            subject: lp.subject,
            topic: lp.topic,
        ...
        // Wait for all seeding writes to finish in parallel
  ...
  // 1. Normalize current logged-in user name once and keep cached
  const normalizedUser = useMemo(() => {
    return normalizeTeacherName(currentUser.name || '').toLowerCase().trim();
  }, [currentUser.name]);

  // 2. Pre-process rawSchedules to find classes and subjects assigned to this teacher with O(M) time complexity
  const userAssignedClassesAndSubjects = useMemo(() => {
    const matchedSet = new Set<string>();
    rawSchedules.forEach((s) => {
      if (!s.guru) return;
      // Split and normalize with O(1) loop checks instead of calculating repeatedly
    ...
    return matchedSet;
  }, [rawSchedules, normalizedUser]);

  // 3. Filter lesson plans with O(N) complexity instead of O(N * M)
  const activeUserPlans = useMemo(() => {
    return lessonPlans.filter((lp) => {
      ...
      return userAssignedClassesAndSubjects.has(`${classKey}_${subjectKey}`);
    });
  }, [lessonPlans, userAssignedClassesAndSubjects]);

  // 4. Memoize the fallback dashboard plans
  const dashboardPlans = useMemo(() => {
    if (activeUserPlans.length > 0) return activeUserPlans;
    return lessonPlans.filter((lp) => 
      (lp.class && (lp.class.includes("10 ME") || lp.class.includes("11 IPA 1")))
    );
  }, [activeUserPlans, lessonPlans]);
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-6 py-6 md:py-8 flex flex-col gap-6">
        ...
        {activeNav === 'dashboard' ? (
          ...
          <MonitoringView 
            ...
            currentUserName={currentUser.name}
          />
        ) : isScheduleDashboardEnabled && activeNav === 'jadwal' ? (
