...
export default function ScheduleDashboardView({ schedules = [], currentUserName = 'Indra Bayu Muktias' }: ScheduleDashboardViewProps) {
  ...
  const [activeItem, setActiveItem] = useState<{ type: 'guru' | 'kelas'; name: string } | null>(initialItem);

  // Sync activeItem when schedules are fetched or list updates
  useEffect(() => {
    if (!activeItem) {
      if (initialItem) {
              ...
              <div className="flex flex-col gap-1.5 w-full max-w-xl relative">
                ...
                <div className="relative">
                  <input
                    ...
                    type="text"
                    placeholder="Cari nama ustadz / ustadzah atau kelas..."
                    value={searchQuery}
                    onChange={(e) => {
                ...
                <div className="bg-white/50 border border-[#e5e7eb] p-4 rounded-xl text-xs font-semibold text-on-surface-variant flex items-start gap-2.5 leading-relaxed shadow-xs">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] leading-normal">
                      Pilih nama Ustadz/Ustadzah dari kolom pencarian di atas untuk memuat jadwal mengajar beliau. 
